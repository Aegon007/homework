...
def convert_2_float(binary_rep):
...
def convert_bin_2_deci(x_bin, int_bit_len):
    ...
    x_int = convert_2_int(x_bin_int)
    x_float = convert_2_float(x_bin_float)
    x_val = x_int + x_float
    return x_val
...
class Individual():
    ...
    def calculate_phenotype(self):
        ...
        x_val = np.clip(x_val, *DNA_BOUND)
        y_val = np.clip(y_val, *DNA_BOUND)
        ...
        fxy = (x_val**2 + y_val - 11)**2 + (x_val + y_val**2 - 7)**2
...
class Population():
    ...
    def compute_diversity(self):
        def compute_distance(individual_a, individual_b):
            gene_a, gene_b = individual_a.value, individual_b.value
